...
const PatientD = () => {
  ...
  const isAppointmentDateValid = (appointmentDate) => {
    const oneDayInMillis = 24 * 60 * 60 * 1000;
    const today = new Date();
    const appointmentDateTime = new Date(appointmentDate);
    return appointmentDateTime.getTime() - today.getTime() >= -oneDayInMillis;
  };
              ...
              <Table className="tablesorter" responsive>
                ...
                <tbody>
                  {currentPatients.map((patient, index) => {
                    const isValidAppointmentDate = isAppointmentDateValid(patient.pDate);
                    if (!isValidAppointmentDate) {
                      return alert('No new appointments'); // Skip rendering if the appointment date is older than 1 day
                    }
